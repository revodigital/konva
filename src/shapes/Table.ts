...
export class Table extends Shape<TableConfig> {
   ...
   */
  public externalBorder: IBorderOptions;

  /**
   * Configuration for the internal border of this table
   */
  public internalBorder: IBorderOptions;

  /**
   * The height percentage of the header row
   */
  public headerHeight: number | 'auto';

  /**
   * Fill color for the background of the header
   */
  public headerFill: string;

  /**
   * Header text options
   */
  public headerText: ITextConfiguration;
  ...
  /**
   * Creates a new instance of Table class
   * @param options The configuration for this table
   */
  constructor(options: TableConfig) {
    super(options);
    this.header = Column.fromIColumnArray(options.header);
    this.rows = Row.fromIRowArray(options.rows);

    this.internalBorder = options.internalBorder || BorderOptions.getDefaultBorder();
    this.externalBorder = options.externalBorder || BorderOptions.getDefaultBorder();
    this.tableWidth = this.width() || 40;
    this.tableHeight = this.height() || 40;
    this.headerHeight = options.headerHeight || 'auto';
    this.headerFill = options.headerFill || 'cyan';
    this.headerText = options.headerText || TextConfiguration.getDefaultOptions();
  }

  /**
   * Draws the table borders
   * @param ctx The context to use for drawing
   * @param layout Complete table layout
   * @private
   */
  ...
  _sceneFunc(ctx: Context) {
    // Save a copy of the old rows
    const rows_temp = Array.from(this.rows);
    ...
    // Clear all
    this._clearShape(layout, ctx._context);

    // Render content
    this._renderContent(layout, ctx._context);

    // Draw table borders
    this._renderTableBorders(layout, ctx._context);
   ...
   * @private
   */
  private _clearShape(layout: TableLayout, ctx: CanvasRenderingContext2D): void {
    ctx.clearRect(layout.edgesRectangle.topLeft.x,
      layout.edgesRectangle.topLeft.y,
      layout.tableWidth,
      layout.tableHeight);
  }
  ...
  private _calculateLayout(): TableLayout {
    ...
    layout.parseFromConfiguration(ColumnRowLayoutConfiguration.fromColRowConfig(
      this.header,
      this.rows));
    layout.edgesRectangle = PointRectangle.calculateFromCenter(this.tableWidth,
      this.tableHeight);
    layout.tableHeight = this.tableHeight;
    layout.tableWidth = this.tableWidth;
  ...
  public addRow(row: Row, index: number, verse: Verse, include?: boolean): void {
    ...
    if (include === true) {
      ...
      if (rowLayout.heightIsAuto())
        addHeight = this.tableHeight * (RowLayoutGroup.fromRawConfiguration(
          this.rows).getAutoRowPercentage() / 100);
      else
        addHeight = this.tableHeight * ((rowLayout.height as number) / 100);

      this.height(this.tableHeight + addHeight);
      this.tableHeight = (this.tableHeight + addHeight);
    }
  }
  ...
  public addColumn(col: Column, index: number, verse: Verse, include?: boolean, data?: string[]): void {
    ...
    if (include === true) {
      ...
      if (column.widthIsAuto())
        addWidth = this.tableWidth * (ColLayoutGroup.fromRawConfiguration(
          this.header).getAutoColPercentage() / 100);
      else {
        addWidth = this.tableWidth * ((column.width as number) / 100);
      }

      this.width(this.tableWidth + addWidth);
      this.tableWidth = this.tableWidth + addWidth;
    }
  }
  ...
  public setWidth(newValue: number): void {
    if (newValue <= 0) throw new Error(`Invalid width for table ${ this }`);
  ...
  /**
   * Sets a new height for this table
   * @param newValue New height
   */
  public setHeight(newValue: number): void {
    if (newValue <= 0) throw new Error(`Invalid height for table ${ this }`);
  ...
  public scaleWidthByPerc(perc: number): void {
    ...
    this.setWidth((this.tableWidth * (perc / 100)) + this.tableWidth);
  }
  ...
  public scaleHeightByPerc(perc: number): void {
    ...
    this.setHeight((this.tableHeight * (perc / 100)) + this.tableHeight);
  }
  ...
  public downscaleHeightByPerc(perc: number): void {
    ...
    this.setHeight(this.tableHeight - (this.tableHeight * (perc / 100)));
  }
  ...
  public downscaleWidthByPerc(perc: number): void {
    ...
    this.setWidth(this.tableWidth - (this.tableWidth * (perc / 100)));
  }
   ...
   * @private
   */
  private _renderContent(layout: TableLayout, ctx: CanvasRenderingContext2D): void {
    // Drawing start point
    let startingPoint = new Point(layout.edgesRectangle.topLeft.x,
      layout.edgesRectangle.topLeft.y);
    ...
    for (let row = 0; row < layout.getRowsCount(); row++) {
      for (let col = 0; col < layout.getColumnsCount(); col++) {
        ...
        const cellRect = cellLayout.calculateLayout(this.tableHeight,
          this.tableWidth);
        ...
        const rowConfiguration = this.rows[row];
        cellContent = rowConfiguration.data[col];
        cellConf = this.rows[row];
        ...
        // Increment point
        startingPoint = new Point(startingPoint.x + cellLayout.getEffectiveWidth(
          this.tableWidth), startingPoint.y);
      }
  ...
  /**
   * Clones this object
   * @param cb Clone callback. receives the newly created item as first argument
   */
  clone(cb: (item: Table) => void): void {
    // cb(
    //   new Table({ ...this }));
  }
}
